...
const defaultToolbox = [
  {
    id: "niche",
    title: "Niche",
    tags: ["Mapbox", "BabylonJS", "Unity 3D", "3D Web (legacy)"],
  },
  {
    id: "languages",
    title: "Languages & Frameworks",
    tags: ["TypeScript", "React", "Next.js", "React Native", "Python"],
  },
  {
    id: "past-life",
    title: "A past life",
    tags: ["Ruby", "PHP", "Rust", "C++", "C#", "Bash", "Inno Setup"],
  },
  {
    id: "backend",
    title: "Backend & Auth",
    tags: ["PostgreSQL", "Supabase", "Auth0", "Azure Functions", "Stripe"],
  },
  {
    id: "platform-ai",
    title: "Platform & AI",
    tags: ["Monorepos", "MCP", "TanStack", "Expo", "OpenAI"],
  },
  {
    id: "leadership",
    title: "Leading",
    tags: ["Leadership", "Mentorship", "Problem Solving", "Communication"],
  },
];

const defaultSiteSettings = {
  name: "Zaryab Ali",
  profileImage: "/zaryab.png",
  favicon: "/favicon.png",
  cvUrl: "/Zaryab's CV (Software Engineer).pdf",
...
export async function readToolbox() {
  ...
  const list = Array.isArray(parsed)
    ? parsed.map(normalizeToolboxItem)
    : defaultToolbox;
...
export async function readSiteSettings() {
  ...
  return {
    ...
    ...parsed,
    profileImage: String(
      parsed.profileImage || defaultSiteSettings.profileImage,
    ),
    favicon: String(parsed.favicon || defaultSiteSettings.favicon),
    cvUrl: String(parsed.cvUrl || defaultSiteSettings.cvUrl),
    heroSince: parsed.heroSince
      ? String(parsed.heroSince)
      : defaultSiteSettings.heroSince,
    socialLinks:
      Array.isArray(parsed.socialLinks) && parsed.socialLinks.length
    ...
    aboutParagraphs:
      Array.isArray(parsed.aboutParagraphs) && parsed.aboutParagraphs.length
        ? parsed.aboutParagraphs.map((paragraph) => String(paragraph))
        : defaultSiteSettings.aboutParagraphs,
    aboutStats:
      Array.isArray(parsed.aboutStats) && parsed.aboutStats.length
        ? parsed.aboutStats.map((stat) => ({
            value: String(stat?.value || ""),
            label: String(stat?.label || ""),
          }))
        : defaultSiteSettings.aboutStats,
  };
}
...
export async function writeSiteSettings(settings) {
  ...
  const next = {
    ...defaultSiteSettings,
    ...settings,
    heroSince: settings?.heroSince
      ? String(settings.heroSince)
      : defaultSiteSettings.heroSince,
    socialLinks: Array.isArray(settings?.socialLinks)
      ? settings.socialLinks
      : defaultSiteSettings.socialLinks,
    aboutParagraphs: Array.isArray(settings?.aboutParagraphs)
      ? settings.aboutParagraphs
      : defaultSiteSettings.aboutParagraphs,
    aboutStats: Array.isArray(settings?.aboutStats)
      ? settings.aboutStats
      : defaultSiteSettings.aboutStats,
  };
